  ...
  return (
    <div className="mt-4 max-w-2xl animate-fade-up-delay">
      {cruisePositioning.eyebrow ? (
        <p className="mb-1.5 text-[0.7rem] font-medium uppercase tracking-[0.12em] text-autumn-300/95 sm:text-xs">
          {cruisePositioning.eyebrow}
        </p>
      ) : null}
      <p className="flex items-start gap-2.5 text-sm leading-snug text-white/88 sm:text-[0.95rem] sm:leading-relaxed">
        <ShipAnchorIcon className="mt-0.5 h-4 w-4 shrink-0 text-autumn-300/90 sm:mt-1 sm:h-[1.05rem] sm:w-[1.05rem]" />
        <span>{message}</span>
      </p>
    </div>
  );
