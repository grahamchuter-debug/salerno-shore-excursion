#!/usr/bin/env node
/**
 * Post-build SEO QA checks for Salerno Shore Excursion static export
...
if (!existsSync(OUT)) {
  ...
  const indexableMustNotHaveNoindex = htmlFiles.filter(
    (f) =>
      !f.includes("/privacy/") &&
      !f.includes("/terms/") &&
      !f.includes("/signature-tour-terms/") &&
      !f.includes("/port-guide/"),
  );
...
if (existsSync(robots)) {
  ...
  if (isPreviewBuild) {
    if (!/disallow:\s*\//i.test(text)) {
      errors.push("Preview build robots.txt must disallow all crawling");
    }
  } else {
...
if (existsSync(sitemap)) {
  ...
  if (isPreviewBuild && urls.length > 0) {
    errors.push("Preview build must not publish sitemap URLs");
  }
...
} else if (existsSync(OUT) && !isPreviewBuild) {
  errors.push("sitemap.xml missing from out/");
}

// 8. _redirects — Workers Static Assets use relative path aliases only.
// Host-level www → apex is Cloudflare Redirect Rules (see CLOUDFLARE-WORKERS-DEPLOYMENT.md).
...
if (existsSync(redirects)) {
  ...
  // Absolute https://www host rules are rejected by Workers assets — do not require them.
  if (text.includes("https://www.")) {
    errors.push("_redirects must not use absolute www host rules on Workers Static Assets");
  }
} else {
...
console.log("  • /privacy/, /terms/ and /signature-tour-terms/ — noindex, excluded from sitemap");
console.log("  • HTTP→HTTPS — Cloudflare Always Use HTTPS");
console.log("  • WWW→apex — Cloudflare Redirect Rules (not Workers _redirects host rules)");
